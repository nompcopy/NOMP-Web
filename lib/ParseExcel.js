...
function parse_ticket(data, cb) {
    ...
    try {
        ...
        ticket.contact_phone = data[5] === undefined ? '' : data[5].value;
        // index 6, mobile
        ticket.contact_mobile = data[6] === undefined ? '' :  data[6].value;
        // index 7, email
        ticket.contact_email = data[7] === undefined ? '' :  data[7].value;
        // index 8, quantity
        ...
        if (ticket_type == 'need') {
            ticket.budget = data[17].value;
        }
        else if (ticket_type == 'offer') {
            ticket.cost = data[18].value;
        }
        // manage the search of class and actor type
        async.waterfall([
            // find class
            function(callback) {
                ClassificationModel.retrieveByValue(classification.trim(), function(err, classificationObj) {
                    ...
                    ticket.classification_name = classificationObj.name;
                    callback(null, ticket);
                });
            },
            // find source_actor_type
            function(ticket, callback) {
                ActorTypeModel.retrieveByValue(source_actor_type.trim(), function(err, actor_typeObj) {
                    ...
                    ticket.source_actor_type_name = actor_typeObj.name;
                    callback(null, ticket);
                });
            },
            // find target_actor_type
            function(ticket, callback) {
                ActorTypeModel.retrieveByValue(target_actor_type.trim(), function(err, actor_typeObj) {
                    if (actor_typeObj === null) {
                    ...
                    ticket.target_actor_type_name = actor_typeObj.name;
                    callback(null, ticket)
                });
            }
        ], cb);
