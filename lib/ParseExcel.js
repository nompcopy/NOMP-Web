...
function parse_ticket(data, cb) {
    ...
    try {
        ...
        async.waterfall([
            // find class
            function(callback) {
                ClassificationModel.retrieveByValue(classification, function(err, classification) {
                    if (classification === null) {
                        throw "Classification not found for ticket name " + ticket.name;
                    }
                    ticket.classification = classification._id;
                    callback(null, ticket);
                });
            },
            // find source_actor_type
            function(ticket, callback) {
                ActorTypeModel.retrieveByValue(source_actor_type, function(err, actor_type) {
                    if (actor_type === null) {
                        throw "Source Actor Type not found for ticket name " + ticket.name;
                    }
                    ticket.source_actor_type = actor_type._id;
                    callback(null, ticket);
                });
            },
            // find target_actor_type
            function(ticket, callback) {
                ActorTypeModel.retrieveByValue(target_actor_type, function(err, actor_type) {
                    if (actor_type === null) {
                        throw "Target Actor Type not found for ticket name " + ticket.name;
                    }
                    ticket.target_actor_type = actor_type._id;
                    callback(null, ticket)
                });
            }
        ], cb);
